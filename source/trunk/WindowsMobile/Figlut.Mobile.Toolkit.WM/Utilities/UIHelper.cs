...
using Figlut.Mobile.Toolkit.Data.DB;
    ...
    public class UIHelper
    {
        ...
        public static void EnableKeyEventHandler(Form form, KeyEventHandler keyEventHandler)
        {
            if (form != null && keyEventHandler != null)
            {
                Application.DoEvents(); //Flush the message queue, discarding that the user may have caused this code to execute by a key press e.g. by pressing enter.
                form.KeyDown += keyEventHandler;
            }
        }
        ...
        public static void MenuEnabled(MainMenu mainMenu, bool enabled)
        {
            if (mainMenu == null || mainMenu.MenuItems == null)
            {
                return;
            }
        ...
        public static DataGridTableStyle GetDataGridTableStyle(Type entityType, int width, List<string> hiddenColumns, bool shapeColumnNames)
        {
            DataGridTableStyle result = new DataGridTableStyle();
            result.MappingName = shapeColumnNames ? DataShaper.ShapeCamelCaseString(entityType.Name) : entityType.Name;
            List<string> columns = EntityReader.GetAllPropertyNames(shapeColumnNames, entityType);
            foreach (string hc in hiddenColumns)
            {
                result.GridColumnStyles.Add(GetColumnStyle(hc, hc, -1));
            }
            foreach (string c in columns)
            {
                ...
                result.GridColumnStyles.Add(GetColumnStyle(c, c, width));
            }
            return result;
        }
            ...
            List<LinkLabel> resetLinks)
        {
            ...
            for (int i = 0; i < entityProperties.Length; i++)
            {
                ...
                if (resetLinks != null)
                {
                    LinkLabel lnkReset = new LinkLabel() { Name = string.Format("lnk{0}", p.Name), Text = "Reset", Tag = control, Dock = DockStyle.Right, Width = 60 };
        ...
        public static void ClearControls(Dictionary<string, Control> controls)
        {
            foreach (Control control in controls.Values)
            {
                ...
                //}
                Panel pnlInputControl = control as Panel;
                Control inputControl = null;
                if (pnlInputControl != null)
                {
                    foreach (Control c in pnlInputControl.Controls)
                    {
                        if (!(c is LinkLabel))
        ...
        {
            string errorMessage;
            return AllControlsPopulated(controls, focusOnBlankControl, displayErrorMessage, out errorMessage, form, keyEventHandler);
        }
            ...
            KeyEventHandler keyEventHandler)
        {
            ...
            return AllControlsPopulated(controlsList, focusOnBlankControl, displayErrorMessage, out errorMessage, form, keyEventHandler);
        }
            ...
            KeyEventHandler keyEventHandler)
        {
            ...
            if (blankControl != null)
            {
                ...
                {
                    UIHelper.DisplayError(errorMessage, form, keyEventHandler);
                }
                return false;
            }
            return true;
