namespace Figlut.Mobile.Toolkit.Web.Client.FiglutWebService
{
    ...
    using Figlut.Mobile.Toolkit.Data.DB.Schema;
    ...
    public class FiglutWebServiceClientWrapper
    {
        ...
        public SqlCeDatabase GetSqlDatabase(
            ...
            bool wrapWebException)
        {
            ...
            SqlCeDatabase result = new SqlCeDatabase();
            HttpStatusCode statusCode;
            ...
            {
                SqlCeDatabaseTable table = new SqlCeDatabaseTable(t);
                ...
                if (result.Tables.Exists(table.TableName))
                {
                    throw new Exception(string.Format(
                        ...
                        typeof(SqlCeDatabaseTable).FullName,
                        table.TableName));
                }
                ...
                foreach (ForeignKeyInfo foreignKeyColumn in tableForeignKeys.ForeignKeys)
                {
                    ...
                    MobileDatabaseTableColumn c = table.Columns[foreignKeyColumn.ChildTableForeignKeyName];
                    c.IsForeignKey = true;
        ...
        private void PopulateSqlDatabaseChildrenTables(SqlCeDatabase database)
        {
            foreach (SqlCeDatabaseTable pkTable in database.Tables)
            {
                foreach (SqlCeDatabaseTable fkTable in database.Tables) //Find children tables i.e. tables that have foreign keys mapped this table's primary keys'.
                {
            ...
            bool wrapWebException)
        {
            string queryString =
                ...
                string.Format("{0}", handlerName) :
                string.Format("{0}/{1}", handlerName, customParameters);
            return _webServiceClient.CallService<T>(
                queryString,
                requestPostObject,
                HttpVerb.POST,
