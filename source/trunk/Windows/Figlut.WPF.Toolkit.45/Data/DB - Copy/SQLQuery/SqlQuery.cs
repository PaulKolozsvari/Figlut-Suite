    ...
    public class SqlQuery
    {
        ...
        public void AppendWhereColumns(List<WhereClauseColumn> whereClause)
        {
            ...
            foreach (WhereClauseColumn whereColumn in whereClause)
            {
                ...
                {
                    _sqlParameters.Add(new SqlParameter(string.Format("@{0}", whereColumnName), whereColumn.ColumnValue));
                    _sqlQueryString.Append(string.Format("[{0}] {1} @{0}", whereColumnName, whereColumn.ComparisonOperator.ToString()));
                }
                else
                {
